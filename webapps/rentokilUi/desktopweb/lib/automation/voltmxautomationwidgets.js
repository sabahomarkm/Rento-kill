(function() {
    ...
    var textbox_enterText = function() {
        ...
        if(typeof arguments[1] !== 'string' && !$KU.is(arguments[1], 'array') && (widgetModel.textInputMode !== "numeric" && typeof arguments[1] !== 'number')) {
                $KAUtils.throwExceptionInvalidArgumentType();
        }
        ...
        if(arguments.length == 2 && typeof arguments[1] == 'string' ||  (widgetModel.textInputMode === "numeric" && typeof arguments[1] === 'number')) {
            const maxLen = widgetModel.maxTextLength;
            let textToEnter = arguments[1];
            if (typeof maxLen === 'number' && maxLen >= 0) {
                if (widgetModel.textInputMode === 'numeric') {
                    if (/^\d*$/.test(textToEnter)) {
                        textToEnter = String(textToEnter);
                        if (textToEnter.length > widgetModel.maxTextLength) {
                            textToEnter = textToEnter.slice(0, widgetModel.maxTextLength);
                        }
                    } else {
                        textToEnter = "";
                    }
