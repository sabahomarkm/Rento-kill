(function() {
    ...
    var _valid = {
        TextBox2: {
            ...
            autoCompleteType: function TextBox2$_valid_name(value) {
                var $K = voltmx.$kwebfw$, $KU = $K.utils, flag = false;
    ...
    var _view = {
        TextBox2: {
            ...
            maxTextLength: function TextBox2$_view_maxTextLength(el/* , old */) {
                ...
                } else if (this.textInputMode === "any" && typeof this.maxTextLength === "number" && this.maxTextLength > 0) {
                    $KD.setAttr(el.node, 'maxlength', this.maxTextLength);
                }

                // Always apply numeric key blocking if textInputMode is 'numeric'
                if (this.textInputMode === "numeric" && !el.node._numericListenerAttached) {
                    ...
                    el.node.addEventListener("input", function (e) {
                        let val = e.target.value;
                        if (typeof this.maxTextLength === "number" && this.maxTextLength >= 0 && val.length > this.maxTextLength) {
                            val = val.slice(0, this.maxTextLength);
                        }
                        e.target.value = val;
                        this.text = val;
                    }.bind(this));
                    el.node._numericListenerAttached = true;
                }
            },
            ...
            text: function TextBox2$_view_text(el/* , old */) {
                ...
                if (value !== this.text) {
                    let updatedText = String(this.text);
                    // Truncate only if maxTextLength is a valid number (>= 0)
                    if (typeof maxLen === 'number' && maxLen >= 0) {
                        if (this.textInputMode === 'numeric') {
                            if (/^[+-]?(\d+(\.\d*)?|\.\d+)([eE][+-]?\d+)?$/.test(updatedText)) {
                                if (updatedText.length > this.maxTextLength) {
                                    updatedText = updatedText.slice(0, this.maxTextLength);
                                }
                            } else {
                                updatedText = "";
                            }
            ...
            if(this.isVisible || $K.F.RIVW) {
                if(!view) {
                    ...
                    if(this.autoCompleteType) {
                        $KD.setAttr(view, 'name', this.autoCompleteType);
                    }                    
