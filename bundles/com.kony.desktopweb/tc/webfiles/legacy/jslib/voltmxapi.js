...
$KI.setapplicationcallbacks = function (callbacksObj) {
    $KU.logExecuting('voltmx.application.setApplicationCallbacks');
    $KU.logExecutingWithParams('voltmx.application.setApplicationCallbacks', callbacksObj);
    ...
    $KU.logExecutingFinished('voltmx.application.setApplicationCallbacks');    
};
...
voltmx.image = {
    ...
    createImageFromSnapShot: function(widget) {
        var elm = $KU.getNodeByModel(widget);
        if (!elm) {
        ...
        let client = elm.getBoundingClientRect();
        ...
        ctx.fillStyle = getComputedStyle(elm).backgroundColor;
        ctx.fillRect(0, 0, canvas.width, canvas.height);
    
        if (elm.tagName === 'IMG') {
            ctx.drawImage(elm, 0, 0, canvas.width, canvas.height);
        }
    
        ctx.fillStyle = getComputedStyle(elm).color;
        ctx.font = `${getComputedStyle(elm).fontSize} ${getComputedStyle(elm).fontFamily}`;
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        let txt = elm.innerText || elm.textContent || elm.value || '';
