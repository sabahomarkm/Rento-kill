...
voltmx.automation.textbox = (function() {
    ...
    var module = {
        enterText: function(widgetPath, item) {
            ...
            if(typeof item !== 'string' && (widgetModel.textInputMode !== "N" && typeof item !== 'number')) {
                $KAUtils.throwExceptionInvalidArgumentType();
            }
            var node = null;
            ...
            if (typeof maxLen === 'number' && maxLen >= 0) {
                if (widgetModel.textInputMode === 'N') {
                    if (/^\d*$/.test(item)) {
                        item = String(item);
                        if (item.length > widgetModel.maxTextLength) {
                            item = item.slice(0, widgetModel.maxTextLength);
                        }
                    } else {
                        item = "";
                    }
