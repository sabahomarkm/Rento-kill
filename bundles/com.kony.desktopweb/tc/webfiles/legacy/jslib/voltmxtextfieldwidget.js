...
$KW.TextField = (function() {
    ...
    var module = {
            ...
            switch(propertyName) {
                case "text":
                    if(propertyValue) {
                        ...
                        if (widgetModel.textInputMode === 'N' && typeof maxLen === "number" && maxLen >= 0) {
                            let val = String(propertyValue);
                            if (/^[+-]?(\d+(\.\d*)?|\.\d+)([eE][+-]?\d+)?$/.test(val)) {
                                if (val.length > widgetModel.maxTextLength) {
                                    val = val.slice(0, widgetModel.maxTextLength);
                                }
                            } else {
                                val = "";
                            }
                            ...
                            element.value = (typeof maxLen === "number" && maxLen >= 0) ? value.substring(0, maxLen) : propertyValue;
                        }
        ...
        render: function(textModel, context) {
            ...
            htmlString += textModel.autoComplete === true ? " autocomplete='on'" : textModel.autoComplete === false ? " autocomplete='off'" : ` autocomplete='${textModel.autoComplete}'`;
            htmlString += textModel.autoCompleteType ?  " name='" + textModel.autoCompleteType + "'" : "";
        ...
        textfieldGoButtonEventHandler: function(eventObject, target) {
            ...
            if (textModel.textInputMode === 'N') {
                const allowedKeys = [
                    "Backspace", "Delete", "ArrowLeft", "ArrowRight", "Tab",
                    "Home", "End", "ArrowUp", "ArrowDown"
                ];
                if (allowedKeys.includes(eventObject.key) || (eventObject.ctrlKey || eventObject.metaKey)) {
                    return;
                }
                if (!/^\d$/.test(eventObject.key)) {
        ...
        textfieldInputEventHandler: function(eventObject, target) {
            ...
            if (textModel.textInputMode === "N") {
                ...
                if (typeof textModel.maxTextLength === "number" && textModel.maxTextLength >= 0 && val.length > textModel.maxTextLength) {
                    val = val.slice(0, textModel.maxTextLength);
                }
                target.value = val;
                textModel.text = val;
            }
