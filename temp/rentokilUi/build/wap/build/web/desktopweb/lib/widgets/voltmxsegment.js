        ...
        if(prop.data && prop.data.length > 0) {
            ...
            for(r=0; r<rlen; r++) {
                ...
                if(!section) {
                    ...
                    prop.selectedItems.push(prop.data[rindex]);
                } else {
                    prop.selectedRowItems.push(prop.data[sindex][1][rindex]);
                    prop.selectedItems.push(prop.data[sindex][1][rindex]);
                }
        ...
        SegmentedUI2: {
            ...
            selectedRowIndex: function SegmentedUI2$_view_selectedRowIndex(el/*, old*/) {
                var $K = voltmx.$kwebfw$, $KW = $K.widget, _ = this._kwebfw_,
                    prop = _.prop, rowIndex = 0, selectedRow = null, $KD = $K.dom;
                ...
                if($KW.visible(this) && prop.selectedRowIndex) {
                    ...
                    if(prop.viewType === constants.SEGUI_VIEW_TYPE_PAGEVIEW) {
                        ...
                        if(_shouldLazyLoad.call(this)) {
                            ...
                            // When retainSelection is enabled, ensure the selected row visually retains its focus skin
                            // even after data or view updates, by applying the focus skin to the corresponding DOM element.
                            if (this.retainSelection) {
                                let listElement = $KD.closest(selectedRow._kwebfw_.view, 'kr', 'item');
                                if (listElement) {
                                    if (this.viewType === constants.SEGUI_VIEW_TYPE_TABLEVIEW && this.rowFocusSkin) {
                                        $KD.setAttr(listElement, 'class', this.rowFocusSkin);
                                    }
                                }
                            }
                        }
